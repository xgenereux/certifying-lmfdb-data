...
lemma square_and_multiply_algorithm {R : Type*} [Monoid R] {s t : ℕ} (y : Fin (s + 1) → R)
  ...
  | succ s hsucc =>
    ...
    have harg1 : y' (Fin.last s) = x ^ (bit' (Fin.last s) : ℕ) := by
      convert hs <;> exact Fin.succ_last _
    have harg2 : ∀ (i : Fin s), y' (Fin.castSucc i) =
        (y' (Fin.succ i)) ^ t * x ^ (bit' (Fin.castSucc i) : ℕ) := by
      intro i
      exact hmul (Fin.succ i)
    have aux2 := hsucc y' bit' harg1 harg2
    rw [← aux2]
    exact (hmul 0)
