...
theorem Finsupp.apply_linearCombination' {α : Type*} (f : M →ₛₗ[g] N) (v) (l : α →₀ R) :
  ...
  | zero => simp only [map_zero, mapRange_zero]
  | add x y hx hy =>
...
theorem Submodule.finsupp_mem_smul_top (x : ι →₀ R) (I : Ideal R)
  ...
  case _ i b x' hix' _ ih =>
    ...
    · specialize h i
      ...
      rw [Finsupp.add_apply, Finsupp.notMem_support_iff.mp hix', add_zero, Finsupp.single_eq_same] at h
      exact Submodule.smul_mem_smul h mem_top
    · intro j
      by_cases hij : i = j
      · subst hij
        rw [Finsupp.notMem_support_iff.mp hix']
        apply zero_mem
      specialize h j
      rwa [Finsupp.add_apply, Finsupp.single_eq_of_ne (Ne.intro fun a ↦ hij (id (Eq.symm a))),
        zero_add] at h
...
theorem LinearIndependent.mapₛₗ' {τ : Type* } {v : ι → M} (b : Basis τ R M) (φ : ι → τ)
  ...
  · simp only [Function.comp_apply, Basis.coord_apply, Basis.repr_self,
    Finsupp.single_eq_same, smul_eq_mul, mul_one]
  · intros j _ hjnei
    have := Function.Injective.ne phi_inj hjnei
    simp only [Ne.intro fun a ↦ hjnei (phi_inj (id (Eq.symm a))),
      Function.comp_apply, Basis.coord_apply, Basis.repr_self, Finsupp.single_eq_of_ne,
    ne_eq, not_false_iff, smul_eq_mul, mul_zero]
  · intro hins
    rw [Finsupp.notMem_support_iff.1 hins , zero_smul]
  · intros h _
    rw [zero_smul]
...
lemma Basis.comp_semilinear_repr
  ...
  intro i
  unfold Finsupp.sum
  simp only [map_sum, map_smul, Module.Basis.repr_self, Finsupp.smul_single, smul_eq_mul, mul_one,
     Finset.sum_apply' i]
...
noncomputable def basisSubmoduleModOfBasisMod  { R S M N J: Type _} {n m : ℕ} {p : R} [CommRing R]
    [IsCancelMulZero R] [Field S] [AddCommGroup M] [AddCommGroup N] [AddCommGroup J] [Nonempty (Fin m ⊕ Fin n)]
    [Module R M] [Module S N] [hN : IsTorsionFree R M] {g : R →+* S}
    ...
    (h : I →ₛₗ[g] J) (h' : J → I) (hh : Function.RightInverse h' h)
    (hker2 : LinearMap.ker h ≤ RingHom.ker g • ⊤)
  ...
  have hi : LinearIndependent S v := by
    ...
    replace ht_copy := (smul_eq_zero_iff_right hpnez).1 ht_copy
    ...
    have hcr_z : ∀ x , c (Sum.inr x) = 0 := by
      ...
      · intros b _ hbnex
        ...
        exact Ne.intro (id (Ne.symm hbnex))
