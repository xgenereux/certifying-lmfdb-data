...
lemma IsRadicalPart_dvd {M : Type*} [CommMonoidWithZero M] [NormalizationMonoid M]
    [UniqueFactorizationMonoid M] {a b c : M} (h1 : IsRadicalPart b a)
    (h2 : IsRadicalPart c a) : c ∣ b := by
  by_cases hM : Nontrivial M
  · have hb : b ≠ 0 := Squarefree.ne_zero h1.2
    have hc : c ≠ 0 := Squarefree.ne_zero h2.2
    rw [UniqueFactorizationMonoid.dvd_iff_normalizedFactors_le_normalizedFactors hc hb]
    have := (UniqueFactorizationMonoid.squarefree_iff_nodup_normalizedFactors hc).1 h2.2
    rw [Multiset.le_iff_subset this]
    rintro p hp
    rw [UniqueFactorizationMonoid.mem_normalizedFactors_iff'] at hp ⊢
    refine ⟨hp.1, ⟨hp.2.1, ?_ ⟩  ⟩
    exact (h1.1 p (Irreducible.prime hp.1)).1 ((h2.1 p (Irreducible.prime hp.1)).2 hp.2.2)
    exact hb ; exact hc
  · rw [nontrivial_iff] at hM
    push Not at hM
    rw [hM c b]

lemma IsRadicalPart_unique {M : Type*} [CommMonoidWithZero M] [NormalizationMonoid M]
    [UniqueFactorizationMonoid M] {a b c : M} (h1 : IsRadicalPart b a)
    (h2 : IsRadicalPart c a) : Associated c b := by
  refine associated_of_dvd_dvd ?_ ?_
  exact IsRadicalPart_dvd h1 h2
  exact IsRadicalPart_dvd h2 h1

/-- Mathlib now has the definition of the radical in a ufd with a normalization.
  We prove that it satisfies the predicate `IsRadicalPart`. -/
lemma radical_isRadicalPart {M : Type*} [CommMonoidWithZero M]
  [NormalizationMonoid M] [UniqueFactorizationMonoid M] {a : M} (ha : a ≠ 0) :
  IsRadicalPart (radical a) a := by
  constructor
  · intro p hp
    symm
    refine UniqueFactorizationMonoid.dvd_radical_iff (Prime.isRadical hp) ha
  · exact UniqueFactorizationMonoid.squarefree_radical


/-- We prove that the radical is associated to an element satisfying the predicate `IsRadicalPart`. -/
lemma IsRadicalPart_associated_radical {M : Type*} [CommMonoidWithZero M]
  [NormalizationMonoid M] [UniqueFactorizationMonoid M] {a b : M} (ha : a ≠ 0)
    (h : IsRadicalPart b a) : Associated b (radical a) := by
  exact IsRadicalPart_unique (radical_isRadicalPart ha) h
