...
lemma mul_def_of_table {ι R S : Type _} [Fintype ι] [Semiring R]
  ...
  · simp
  · intros k hk
    simp only [Basis.repr_self, Finsupp.single_eq_same, ne_eq, hk, not_false_eq_true,
      Finsupp.single_eq_of_ne, mul_zero, zero_mul]
  · intros k hk
    simp only [Basis.repr_self, ne_eq, hk, not_false_eq_true, Finsupp.single_eq_of_ne, zero_mul,
...
theorem PowerBasis.repr_aeval_gen_of_natDegree_lt {R S : Type _} [CommRing R] [Ring S] [Algebra R S]
  ...
  · simp only [Finset.mem_range]
    intros b hb hk
    rw [pb.repr_gen_pow _ , Finsupp.single_eq_of_ne, mul_zero]
    · intro heq
  ...
  · simp only [Finset.mem_range]
    ...
    rwa [Nat.lt_succ_iff, not_le] at hk
...
theorem PowerBasis.repr_aeval_gen {R S : Type _} [CommRing R] [Ring S] [Algebra R S]
  ...
  conv_lhs => rw [← modByMonic_add_div f (minpoly _ (pb.gen))]
