...
lemma list_sum_length (hn : n ≠ 0) (f : Fin n → List R) (h : ∀ i, (f i).length = m) :
  ...
  | succ n hn1 =>
    ...
    by_cases h1 : n = 0
    · simp only [h1, List.ofFn_zero, List.foldr_nil, sup_eq_left]
...
lemma table_add_list_eq_add {S : Type*} [AddCommMonoid S] [Module R S] [Mul S]
  ...
  simp only [ Basis.equivFun_symm_eq_repr_symm' B]
  apply_fun B.repr
  simp only [Basis.repr_symm_apply, map_add, Basis.repr_linearCombination]
  ext k
  simp only [Finsupp.coe_add, Pi.add_apply, Finsupp.coe_equivFunOnFinite_symm,
...
lemma table_mulPointwise_eq_smul
  ...
  ext k
  simp only [Finsupp.coe_smul, Pi.smul_apply, Finsupp.coe_equivFunOnFinite_symm]
...
lemma table_sum_smul {S : Type*} {m : ℕ} [NonUnitalNonAssocSemiring S] [Module R S]
  ...
  · simp only [Basis.equivFun_symm_eq_repr_symm' B]
    apply_fun B.repr
    simp only [Basis.repr_symm_apply, map_sum, map_smul, Basis.repr_linearCombination]
    simp_rw [List.mulPointwise_ofFn, ← List.sum_ofFn' (Nat.succ_ne_zero _), List.ofFn_inj] at hc
    ext k
    rw [hc]
    simp only [Nat.succ_eq_add_one, Finsupp.coe_equivFunOnFinite_symm, Finset.sum_apply]
    exact Eq.symm (Finsupp.finset_sum_apply Finset.univ
...
lemma table_polynomial_eval {S : Type*} [Semiring S] [Module R S] [SMulCommClass R S S]
    ...
    let x : Fin (L.length) → (Fin n → R) := fun i =>
      ...
      (by rw [List.mulPointwise_length, nPow_sq_table_length T' T _ i.val
        ((Fin.pos_iff_ne_zero' _).2 h) heq (
        List.length_ofFn)])
    have hpola : (fun (i : Fin (L.length)) => if i = 0 then List.mulPointwise (L[i]) (List.ofFn z)
      else List.mulPointwise (L[i]) (nPow_sq_table T (List.ofFn a) i) ) =
        fun i => List.ofFn (x i) := by
      apply funext ?_
      intro i
      by_cases hz : i = 0
    ...
    · simp only [Fin.getElem_fin, hc, ↓reduceDIte, x]
      rw [← table_mulPointwise_eq_smul B (FnOfList n (nPow_sq_table T (List.ofFn a) i)
         (nPow_sq_table_length T' T _ i.val ((Fin.pos_iff_ne_zero' _).2 hc) heq (
        List.length_ofFn)) ) _ L[i]]
      · congr
        symm
        rw [table_nPow_sq_table_eq_pow T' T B _
          basisMulBasis heq _ ((Fin.pos_iff_ne_zero' _).2 hc) _]
        exact listOfFn_of_FnOfList n _ _
